...
with tf.Session() as sess:
    ...
    for i in range(config.pairwiseReg_n_epochs):
        ...
        summary_str, _, loss_epoch, strong_loss_epoch, pair_loss_epoch = sess.run([merged_summary_op, optimizer, loss, strong_loss, pair_loss], feed_dict=feed_dict)
        summary_writer.add_summary(summary_str, global_step=global_step.eval())
        if not (i%100):
            print('Epoch: {0} loss: {1:.4f} strong_loss: {2:.4f} pair_loss: {3:.4f}'.format(i, loss_epoch, strong_loss_epoch, pair_loss_epoch))
